...
function GetDetails(id) {
    $.ajax({
        url: "/TerritoryTable/GetDetail/" + id,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (response) {
            $('#TerritoryID').val(response.TerritoryID);
            $('#TerritoryDescription').val(response.TerritoryDescription);
            $('#RegionRegionID').val(response.RegionRegionID);
        },
...
$(document).ready(function () {
    ...
    var table = $('#mainTable').DataTable({
        ...
        ajax: '/TerritoryTable/GetAllRows',
        "autoWidth": false,
        "columnDefs": [
        {
            "searchable": false,
    ...
    $('#mainForm').on('submit', function (event) {
        ...
        var _url = '';
        if ($('#mainModal').data('mode') == 'create') {
            _url = 'TerritoryTable/Create'
        } else {
            _url = 'TerritoryTable/Update'
        }
        if ($('#mainForm').valid()) {
            $.ajax({
                url: _url,
