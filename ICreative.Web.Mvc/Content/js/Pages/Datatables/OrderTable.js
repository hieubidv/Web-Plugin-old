...
function GetDetails(id) {
    $.ajax({
        url: "/OrderTable/GetDetail/" + id,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (response) {
            $('#OrderID').val(response.OrderID);
            $('#CustomerCustomerID').val(response.CustomerCustomerID);
            $('#EmployeeEmployeeID').val(response.EmployeeEmployeeID);
            $('#OrderDate').val(moment(response.OrderDate).format('DD/MM/YYYY'));
            $('#RequiredDate').val(moment(response.RequiredDate).format('DD/MM/YYYY'));
            $('#ShippedDate').val(moment(response.ShippedDate).format('DD/MM/YYYY'));
            $('#ShipperShipperID').val(response.ShipperShipperID);
            $('#Freight').val(response.Freight);
            $('#ShipName').val(response.ShipName);
            $('#ShipAddress').val(response.ShipAddress);
            $('#ShipCity').val(response.ShipCity);
            $('#ShipRegion').val(response.ShipRegion);
            $('#ShipPostalCode').val(response.ShipPostalCode);
            $('#ShipCountry').val(response.ShipCountry);
        },
...
$(document).ready(function () {
    ...
    $('#mainForm').on('submit', function (event) {
        ...
        var _url = '';
        if ($('#mainModal').data('mode') == 'create') {
            _url = 'OrderTable/Create'
        } else {
            _url = 'OrderTable/Update'
        }
        if ($('#mainForm').valid()) {
            $.ajax({
                url: _url,
    ...
    function clear() {
            ...
            $('#CustomerID').val('');
            $('#EmployeeID').val('');
            $('#OrderDate').val(moment(Date.now()).format('DD/MM/YYYY'));
            $('#RequiredDate').val(moment(Date.now()).format('DD/MM/YYYY'));
            $('#ShippedDate').val(moment(Date.now()).format('DD/MM/YYYY'));
            $('#ShipVia').val('');
            $('#Freight').val('');
            $('#ShipName').val('');
            $('#ShipAddress').val('');
