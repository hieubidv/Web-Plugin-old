...
function GetDetails(id) {
    $.ajax({
        url: "/MenuItemTable/GetDetail/" + id,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (response) {
            $('#MenuItemId').val(response.MenuItemId);
            $('#MenuItemName').val(response.MenuItemName);
            $('#ParentId').val(response.ParentId);
            $('#MenuItemUrl').val(response.MenuItemUrl);
        },
...
$(document).ready(function () {
    ...
    $('#mainForm').on('submit', function (event) {
        ...
        var _url = '';
        if ($('#mainModal').data('mode') == 'create') {
            _url = 'MenuItemTable/Create'
        } else {
            _url = 'MenuItemTable/Update'
        }
        if ($('#mainForm').valid()) {
            $.ajax({
                url: _url,
