...
function GetDetails(id) {
    $.ajax({
        url: "/PosSimProviderTable/GetDetail/" + id,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (response) {
            $('#SimProviderId').val(response.SimProviderId);
            $('#SimProviderName').val(response.SimProviderName);
        },
...
$(document).ready(function () {
    ...
    var table = $('#mainTable').DataTable({
        ...
        ajax: '/PosSimProviderTable/GetAllRows',
        "autoWidth": false,
        "columnDefs": [
        {
            "searchable": false,
    ...
    $('#mainForm').on('submit', function (event) {
        ...
        var _url = '';
        if ($('#mainModal').data('mode') == 'create') {
            _url = 'PosSimProviderTable/Create'
        } else {
            _url = 'PosSimProviderTable/Update'
        }
        if ($('#mainForm').valid()) {
            $.ajax({
                url: _url,
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    ...
                    $.ajax({
                        url: 'PosSimProviderTable/Delete',
                        type: 'POST',
                        data: JSON.stringify({
                            "id": id
                        }),
    ...
    $('#upload').on('click', function () {
        ...
        $.ajax({
            url: 'PosSimProviderTable/UploadFile',
