...
function GetDetails(id) {
    $.ajax({
        url: "/PosTerminalTable/GetDetail/" + id,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (response) {
            $('#TerminalId').val(response.TerminalId);
            $('#TerminalIdByHeadQuater').val(response.TerminalIdByHeadQuater);
            $('#InitializeCode').val(response.InitializeCode);
            $('#PosDeviceDeviceId').val(response.PosDeviceDeviceId);
            $('#PosStatusTerminalTerminalStatusId').val(response.PosStatusTerminalTerminalStatusId);
            $('#PosSimSimId').val(response.PosSimSimId);
            $('#ConnectType').val(response.ConnectType);
        },
...
$(document).ready(function () {
    ...
    var table = $('#mainTable').DataTable({
        ...
        ajax: '/PosTerminalTable/GetAllRows',
        "autoWidth": false,
        "columnDefs": [
        {
            "searchable": false,
    ...
    $('#mainForm').on('submit', function (event) {
        ...
        var _url = '';
        if ($('#mainModal').data('mode') == 'create') {
            _url = 'PosTerminalTable/Create'
        } else {
            _url = 'PosTerminalTable/Update'
        }
        if ($('#mainForm').valid()) {
            $.ajax({
                url: _url,
    ...
    function clear() {
            $('#TerminalId').val('');
            $('#TerminalIdByHeadQuater').val('');
            $('#InitializeCode').val('');
            $('#DeviceId').val('');
            $('#TerminalStatusId').val('');
            $('#SimId').val('');
            $('#ConnectType').val('');
    }
    ...
    $('#upload').on('click', function () {
        ...
        $.ajax({
            url: 'PosTerminalTable/UploadFile',
