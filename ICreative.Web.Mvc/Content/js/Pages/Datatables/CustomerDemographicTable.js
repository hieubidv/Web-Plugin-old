...
function GetDetails(id) {
    $.ajax({
        url: "/CustomerDemographicTable/GetDetail/" + id,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (response) {
            $('#CustomerTypeID').val(response.CustomerTypeID);
            $('#CustomerDesc').val(response.CustomerDesc);
        },
...
$(document).ready(function () {
    ...
    var table = $('#mainTable').DataTable({
        ...
        ajax: '/CustomerDemographicTable/GetAllRows',
        "autoWidth": false,
        "columnDefs": [
        {
            "searchable": false,
    ...
    $('#mainForm').on('submit', function (event) {
        ...
        var _url = '';
        if ($('#mainModal').data('mode') == 'create') {
            _url = 'CustomerDemographicTable/Create'
        } else {
            _url = 'CustomerDemographicTable/Update'
        }
        if ($('#mainForm').valid()) {
            $.ajax({
                url: _url,
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    ...
                    $.ajax({
                        url: 'CustomerDemographicTable/Delete',
                        type: 'POST',
                        data: JSON.stringify({
                            "id": id
                        }),
    ...
    $('#upload').on('click', function () {
        ...
        $.ajax({
            url: 'CustomerDemographicTable/UploadFile',
