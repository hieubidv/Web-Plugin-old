...
function GetDetails(id) {
    $.ajax({
        url: "/PosStatusTerminalTable/GetDetail/" + id,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (response) {
            $('#TerminalStatusId').val(response.TerminalStatusId);
...
$(document).ready(function () {
    ...
    var table = $('#mainTable').DataTable({
        ...
        ajax: '/PosStatusTerminalTable/GetAllRows',
        "autoWidth": false,
        "columnDefs": [
        {
            "searchable": false,
    ...
    $('#mainForm').on('submit', function (event) {
        ...
        var _url = '';
        if ($('#mainModal').data('mode') == 'create') {
            _url = 'PosStatusTerminalTable/Create'
        } else {
            _url = 'PosStatusTerminalTable/Update'
        }
        if ($('#mainForm').valid()) {
            $.ajax({
                url: _url,
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    ...
                    $.ajax({
                        url: 'PosStatusTerminalTable/Delete',
                        type: 'POST',
                        data: JSON.stringify({
                            "id": id
                        }),
    ...
    $('#upload').on('click', function () {
        ...
        $.ajax({
            url: 'PosStatusTerminalTable/UploadFile',
