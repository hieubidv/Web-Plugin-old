...
function GetDetails(id) {
    $.ajax({
        url: "/RegionTable/GetDetail/" + id,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (response) {
            $('#RegionID').val(response.RegionID);
            $('#RegionDescription').val(response.RegionDescription);
        },
...
$(document).ready(function () {
    ...
    $('#mainForm').on('submit', function (event) {
        ...
        var _url = '';
        if ($('#mainModal').data('mode') == 'create') {
            _url = 'RegionTable/Create'
        } else {
            _url = 'RegionTable/Update'
        }
        if ($('#mainForm').valid()) {
            $.ajax({
                url: _url,
