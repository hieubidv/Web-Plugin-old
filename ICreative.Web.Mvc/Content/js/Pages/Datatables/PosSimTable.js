...
function GetDetails(id) {
    $.ajax({
        url: "/PosSimTable/GetDetail/" + id,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (response) {
            $('#SimId').val(response.SimId);
            $('#SimCode').val(response.SimCode);
            $('#SimPhoneNumber').val(response.SimPhoneNumber);
            $('#PosStatusSimStatusId').val(response.PosStatusSimStatusId);
            $('#AddedDate').val(moment(response.AddedDate).format('DD/MM/YYYY'));
            $('#PosSimProviderSimProviderId').val(response.PosSimProviderSimProviderId);
        },
...
$(document).ready(function () {
    ...
    $('#mainForm').on('submit', function (event) {
        ...
        var _url = '';
        if ($('#mainModal').data('mode') == 'create') {
            _url = 'PosSimTable/Create'
        } else {
            _url = 'PosSimTable/Update'
        }
        if ($('#mainForm').valid()) {
            $.ajax({
                url: _url,
    ...
    function clear() {
            ...
            $('#SimPhoneNumber').val('');
            $('#StatusId').val('');
            $('#AddedDate').val(moment(Date.now()).format('DD/MM/YYYY'));
            $('#SimProviderId').val('');
    }
